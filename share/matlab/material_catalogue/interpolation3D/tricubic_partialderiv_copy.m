...
for j=2:length(a)-1
    for k=2:length(b)-1
        for l = 2:length(c)-1
            dEda(j,k,l)=(E(j+1,k,l)-E(j-1,k,l))/(a(j+1)-a(j-1));
            dEdb(j,k,l)=(E(j,k+1,l)-E(j,k-1,l))/(b(k+1)-b(k-1));
            dEdc(j,k,l)=(E(j,k,l+1)-E(j,k,l-1))/(c(l+1)-c(l-1));
            ...
            /((a(j+1)-a(j-1))*(b(k+1)-b(k-1))*(c(l+1)-c(l-1)));
        end
    end
end
